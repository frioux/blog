...
push: build
	git push --quiet
	s3cmd sync --region us-west-2 --delete-removed --disable-multipart --no-preserve public/ s3://blog.afoolishmanifesto.com | tee $(log) && set-redirects && set-content-types && go run ./bin/busted-urls < $(log) && rm -f $(log)
